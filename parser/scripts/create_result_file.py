...
qs_followers_count = FbGroupModel.objects.filter(followers_count__isnull=False).filter(is_used=False).order_by(
    '-followers_count')
...
print('Records update:', FbGroupModel.objects.filter(pk__in=[g.pk for g in qs]).update(is_used=True))
